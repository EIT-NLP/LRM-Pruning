...
class HFLM(TemplateLM):
    ...
    def _create_model(
        ...
        autogptq: Optional[Union[bool, str]] = False,
        gptqmodel: Optional[bool] = False,
        # Longwei Ding add for cot Baseline evaluation.
        ckpt: Optional[str] = None, # for skipgpt, dllm, mod
        slice_model_path: Optional[str] = None, # for slicegpt
        adapter_path: Optional[str] = None, # for slicegpt
        prune_model_path: Optional[str] = None, # for llm-pruner
        tune_model_path: Optional[str] = None, # for llm-pruner
        **kwargs,
    ) -> None:
        """
        ...
        eval_logger.info(
            ...
            f"ckpt: {ckpt}, "
            f"slice_model_path: {slice_model_path}, "
            f"adapter_path: {adapter_path}, "
            f"prune_model_path: {prune_model_path}, "
            f"tune_model_path: {tune_model_path}"
        )

        if "shortened" in pretrained:
            return None
        if slice_model_path and adapter_path:
            eval_logger.info(
                f"Evaluation: SliceGPT!"
            )
            from peft import LoraConfig, TaskType, get_peft_model,PeftModel
            from transformers import AutoModelForCausalLM, AutoTokenizer
            from .slicegpt import data_utils, gpu_utils, hf_utils, utils
            from .bo_options import lora_target_map
            # sliced_model_path = ckpt
            if "0.2" in slice_model_path: sparsity = 0.2
            elif "0.4" in slice_model_path: sparsity = 0.4
            else: sparsity = 0.6
            eval_logger.info(
                    f"sparsity: {sparsity}"
                )
            ...
            # 加载 LoRA adapter
            # adapter_path = "/code/Baseline/TransformerCompression/slicegpt_results/finetuned_model_0.2_ckpt/checkpoint-10000/"
            self._model = PeftModel.from_pretrained(model, adapter_path)
        if prune_model_path and tune_model_path:
            eval_logger.info(
                f"Evaluation: LLM-Pruner!"
            )
            from peft import PeftModel
            pruned_dict = torch.load(prune_model_path, map_location='cpu', weights_only=False)
            _, model = pruned_dict['tokenizer'], pruned_dict['model']
             # ✅ 添加此段 patch 代码，防止 config 报错
            ...
            return None
        if ckpt is not None and "post_training" in ckpt:
            # if a checkpoint is passed, we import the model from the checkpoint
            # this is used for LORAs and other custom models that are not in HF hub
            from lm_eval.models.functions_utilize import eval_lor_model_import
            self._model = eval_lor_model_import(ckpt, pretrained)
        elif ckpt is not None and "router_all" in ckpt:
            from lm_eval.models.functions_utilize import eval_lora_router_all_model_import
            self._model = eval_lora_router_all_model_import(ckpt, pretrained)
        elif ckpt is not None and "mod" in ckpt:
            # if a checkpoint is passed, we import the model from the checkpoint
            # eval for mod_twice
            # this is used for LORAs and other custom models that are not in HF hub
            from lm_eval.models.functions_utilize import eval_lora_mod_twice_model_import
            if "0.4" in ckpt: # sparsity is 40 %
                eval_logger.info(
                f"custom model ckpt: {ckpt}, sparisy=0.4")
                self._model = eval_lora_mod_twice_model_import(ckpt, pretrained,0.4)
            elif "0.2" in ckpt:
                eval_logger.info(
                f"custom model ckpt: {ckpt}, sparisy=0.2")
                self._model = eval_lora_mod_twice_model_import(ckpt, pretrained,0.2)
        return None
